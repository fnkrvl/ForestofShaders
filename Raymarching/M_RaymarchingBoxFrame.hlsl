...
for (int i = 0; i < 256; i++)
{    
    float dist = sdf.boxFrameRotated(rayOrigin, float3(25, 25, 25), float3(5, 5, 5), sin(time));

    if(dist < 0.01)
    {
        float eps = 0.001;

        float3 normal = normalize(float3(sdf.boxFrameRotated(float3(rayOrigin.x + eps, rayOrigin.y      , rayOrigin.z      ), float3(25, 25, 25), float3(5, 5, 5), sin(time))
                                       - sdf.boxFrameRotated(float3(rayOrigin.x - eps, rayOrigin.y      , rayOrigin.z      ), float3(25, 25, 25), float3(5, 5, 5), sin(time)),
                                         sdf.boxFrameRotated(float3(rayOrigin.x + eps, rayOrigin.y + eps, rayOrigin.z      ), float3(25, 25, 25), float3(5, 5, 5), sin(time))
